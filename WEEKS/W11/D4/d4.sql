...
-- 3.Lets get all the dates that accounts were created. Write a query to select all the create_date
-- from the “customer” table (there should be no duplicates).
-- SELECT DISTINCT create_date from customer
-- 4.Write a query to get all the customer details from the customer table, it should be displayed
...
-- 6.Write a query to get the address, and the phone number of all customers living in the Texas
-- district, these details can be found in the “address” table.
-- SELECT address, phone from address WHERE address.district = 'Texas'
-- 7.Write a query to retrieve all movie details where the movie id is either 15 or 150.
-- SELECT * from film WHERE film_id in (15,150)
-- 8.Write a query which should check if your favorite movie exists in the database. Have your query
-- get the film ID, title, description, length and the rental rate, these details can be found in the “film” table.
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title like '%ladiator%'
...
-- SELECT film_id, title, description, length, rental_rate FROM film WHERE title like 'Gl%'
...
-- SELECT * FROM film ORDER BY replacement_cost OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY
...
-- SELECT customer.customer_id, concat(first_name, ' ', last_name) as customer, payment_date, amount FROM customer
...
-- SELECT customer.customer_id, first_name, last_name, payment_date, amount FROM customer
